    ...
    <>
      <div className="w-screen bg-[#99b9c0] h-2/6 flex  justify-center place-items-center ">
        <div className="py-3.5 px-4 h-1/2 w-1/3">
          <h1 className="lg:text-5xl md:text-4xl sm:text:3xl font-sans font-extrabold text-white">
            Offers for you{" "}
          </h1>
          <p className="text-[#434546] lg:text-xl md:text-md sm:text-sm">
            Explore top deals and offers exclusively for you!
          </p>
        </div>
        <div className="w-1/3 h-full ">
          <img
            src="https://res.cloudinary.com/dwy8lddht/image/upload/v1681892368/3my_b0veu6.jpg"
            className="object-contain h-full max-w-full"
          />
        </div>
        <div className="w:1/3 bg-[#99b9c0]"></div>
      </div>
          ...
          {cardsArray.map((item) => {
            ...
            return (
              <Link
                ...
                key={"/restaurant/" + item.data.data.id}
              >
                <RestaurantCard
                  name={item.data.data.name}
                  cuisines={item.data.data.cuisines}
                  cloudinaryImageId={item.data.data.cloudinaryImageId}
                  costForTwoString={item.data.data.costForTwoString}
                  avgRating={item.data.data.avgRating}
                  aggregatedDiscountInfo={item.data.data.aggregatedDiscountInfo}
                  slaString={item.data.data.slaString}
                />
              </Link>
            );
          })}
